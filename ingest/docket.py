...
def purge_leased_docket_items(region_code, scrape_type):
    ...
    session_results = sessions.get_sessions_with_leased_docket_items(
        region_code, scrape_type)
...
def remove_item_from_session_and_docket(region_code, scrape_type):
    ...
    Args:
        region_code: (string) Region of session to remove from
        scrape_type: (string) Scrape type of session to remove from

    Returns:
        N/A
    """
    # Get the current session, remove and delete its docket item
    session = sessions.get_open_sessions(region_code,
                                         open_only=True,
