...
def add_docket_item_to_current_session(docket_item_name,
    ...
    Args:
        ...
        region_code: (string) Region of session to add to
        scrape_type: (string) Scrape type of session to add to
    ...
    Returns:
        ...
        False if not
    """
    session = get_open_sessions(region_code,
                                open_only=True,
...
def get_sessions_with_leased_docket_items(region_code, scrape_type):
    ...
    Args:
        region_code: (string) Region code to fetch sessions for
        scrape_type: (string) Scrape type to fetch sessions for
    ...
    """
    session_query = ScrapeSession.query()
    session_query = session_query.filter(ScrapeSession.region == region_code)
    session_query = session_query.filter(
        ScrapeSession.scrape_type == scrape_type)
