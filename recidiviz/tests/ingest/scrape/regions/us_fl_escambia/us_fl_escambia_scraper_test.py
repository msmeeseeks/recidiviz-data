...
"""Scraper tests for us_fl_escambia."""
import unittest
from lxml import html

from recidiviz.ingest.models.ingest_info import IngestInfo, Bond, Hold
from recidiviz.ingest.scrape.regions.us_fl_escambia.us_fl_escambia_scraper \
    import UsFlEscambiaScraper
from recidiviz.tests.ingest import fixtures
from recidiviz.tests.utils.base_scraper_test import BaseScraperTest

_PRINT_PAGE_HTML = html.fromstring(
    fixtures.as_string('us_fl_escambia', 'print.html'))


class TestUsFlEscambiaScraper(BaseScraperTest, unittest.TestCase):
    """Only populate_data needs to be tested, since the initial task is a
    single SCRAPE_DATA task."""

    def _init_scraper_and_yaml(self):
        self.scraper = UsFlEscambiaScraper()
    ...
    def test_populate_data(self):
        ...
        b1.create_charge(statute='784.021.1a',
                         name='CHARGE NAME 1',
                         degree='T', level='F',
                         case_number='23456 (ESCAMBIA'
                                     '\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t'
                                     'COUNTY SHERIFFS OFFICE)',
                         bond=Bond(amount='$10000.00'))
        b1.create_charge(statute='893.13.6a',
                         name='CHARGE-2',
                         degree='T',
                         level='F',
                         case_number='12345 (ESCAMBIA'
                                     '\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t'
                                     'COUNTY SHERIFFS OFFICE)',
                         bond=Bond(amount='$5000.00'))
