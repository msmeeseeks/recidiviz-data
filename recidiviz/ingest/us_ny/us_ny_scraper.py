...
class UsNyScraper(BaseScraper):
    ...
    def get_more_tasks(self, content, params):
        ...
        for one_params in params_list:
            if 'post_data' in one_params:
                one_params['post_data'].update(session_vars)
    ...
    def populate_data(self, content, params, ingest_info):
        ...
        for row in content.xpath('//*[@id="ii"]/table[2]/tr')[1:]:
            ...
            # Get the degree
            charge_degree = None
            if charge_name.lower().endswith('1st'):
                charge_degree = ChargeDegree.FIRST
                charge_name = ' '.join(charge_name.split()[:-1])
            elif charge_name.lower().endswith('2nd'):
                charge_degree = ChargeDegree.SECOND
                charge_name = ' '.join(charge_name.split()[:-1])
            elif charge_name.lower().endswith('3rd'):
                charge_degree = ChargeDegree.THIRD
                charge_name = ' '.join(charge_name.split()[:-1])
